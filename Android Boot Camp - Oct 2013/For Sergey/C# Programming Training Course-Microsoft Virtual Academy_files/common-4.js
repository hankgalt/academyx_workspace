...
function HighlightSearchResult(divMessage, isLogged) {
    $(divMessage).addClass("highlight");
	if(isLogged){
		$(divMessage).find(".progress").show();
		$(divMessage).find(".points").hide();
		$(divMessage).find(".level").hide();
	}
}
function UnHighlightSearchResult(divMessage, isLogged) {
    $(divMessage).removeClass("highlight");
	if(isLogged){
		$(divMessage).find(".progress").hide();
		$(divMessage).find(".points").show();
		$(divMessage).find(".level").show();
	}
}
...
function showRatingPopup(cssClass) {
	$('.video-player').hide();
	showPopup(cssClass);
}

function disableRatingPopup(cssClass) {
	$('.video-player').show();
	disablePopup(cssClass);
}

function showFeedbackPopUpMessage(lnkAdminMessageUniqueID){
    $('.video-player').hide();
	showPopup("popUpMessage");
    __doPostBack(lnkAdminMessageUniqueID, '');
}

function EducationDetais_CloseFeedbackPopUp(lnkClosePopUpUniqueID) {
    $('.video-player').show();
	disablePopup($(".popUpMessage"));
    __doPostBack(lnkClosePopUpUniqueID, '')
}

function showFreeActionPopup(cssClass, popUpName, linkType) {
	$('.video-player').hide();
	showPopup(cssClass);
...
//Region: Content download handlers

function openExternalContent(idContentType, idModule, idEducationType) {
	if (lock(mutex)) {
		var params = {
		...
		CallAJAXPost("Handlers/WebMethodsHandler.aspx/DownloadContent", params, OpenExternalContent_onSuccess, OpenContent_onError);
	}
}

function OpenExternalContent_onSuccess(result) {
	if (result["ErrorMessage"]) {
		...
		if (result["ShowPopup"]) {
			switch (result["ShowPopup"]) {
				case "SnoozedFreeActions":
					showFreeActionPopup('snoozedFreeActionsPopUp', 'follow-up prompt', 'o');
					break;
				case "OutOfFreeActions":
					showFreeActionPopup('outOfFreeActionsPopUp', 'initial prompt', 'o');
					break;
			}
			unlock(mutex);
		...
		if(result["RefreshLearningPlanWidget"])
		{
			__doPostBack($('div[id$="upLearningPlanWidget"]').attr('id'), '');
		}
		
		$('div[ctid=' + result['IdContentType'] + '] .module-material-downloaded-message').css('visibility', 'visible')
		
		var educationTypeProgressBar = $('div[id$="control_percentage_principal"]');
        educationTypeProgressBar.css("width",result["EducationTypeProgress"] + "%");
	    if(result["EducationTypeProgress"] >= 100){
		   educationTypeProgressBar.addClass('completed');
	    }
	    $('#number_percentage_principal span').html(result["EducationTypeProgress"]);

		var etLevelProgress = $('.levelBar-' + result['IdLevel'] + ' .progress_advance');
		etLevelProgress.css('width', result['EducationTypeLevelProgress'] + '%');
		if (result['EducationTypeLevelProgress'] >= 100) {
			etLevelProgress.addClass('completed');
		}
		
		RefreshProgressBar(result["Module"], result["ModuleProgress"]);

		if (result["DestinationURL"]) {
			window.open(result["DestinationURL"], "_blank");
		}
		
		if(result["HtmlResponseMslItems"]){
			var html = result["HtmlResponseMslItems"];
			mslItems = $('div[id$="microsoft-learning-documents-container"]');
		
			if ($(mslItems).attr("HtmlResponseMslItems")) {
				$(mslItems).replaceWith(html);
			}
		}
		
		if(result["ShowNextSteps"]){
			$('.msl-next-steps').removeClass('additional-materials-nonvisible');
			$('.msl-link').removeClass('additional-materials-nonvisible');
		}
		
		if(result["ShowMslLearning"]){
			$('.msl-learning').removeClass('additional-materials-nonvisible');
		}
...
function RefreshProgressBar(idModule, moduleProgress) {
	var module = $('.pb-' + idModule);
	module.width(moduleProgress + '%');
	
	if (moduleProgress >= 100){
		module.parent().addClass('completed');
	}
}

function OpenContent_onError(error) {
	unlock(mutex);
}
...
function openEmbeddedVideo(idContentType, idModule, idEducationType) {
	if (lock(mutex)) {
		var params = {
			idContentType: idContentType,
			idModule: idModule,
		...
		CallAJAXPost("Handlers/WebMethodsHandler.aspx/DownloadContent", params, OpenEmbeddedVideo_onSuccess, OpenContent_onError);
	}
}

function OpenEmbeddedVideo_onSuccess(result) {
	if (result["ErrorMessage"]) {
		showNotification(result["ErrorMessage"], "error");
	}
	else {
...
function downloadAdditionalMaterial(idContentType, idModule, idEducationType) {
	if (lock(mutex)) {
		var params = {
			idContentType: idContentType,
			idModule: idModule,
		...
		CallAJAXPost("Handlers/WebMethodsHandler.aspx/DownloadContent", params, OpenAdditionalMaterial_onSuccess, OpenContent_onError);
	}
}

function OpenAdditionalMaterial_onSuccess(result) {
	if (result["ErrorMessage"]) {
		showNotification(result["ErrorMessage"], "error");
	}
	else {
        ...
        if (result["CourseCompleted"]){
		   trackCourseCompletion(result["CourseCompleted"]);
        }
		if (result["ModuleCompleted"]){
		   trackFullEngagement(result["ModuleCompleted"]);
		}
		...
		var moduleProgressBar = $('div[id$="advance"]');
		moduleProgressBar.css('width', result["ModuleProgress"] + '%');
		if(result["ModuleProgress"] == 100){
			moduleProgressBar.addClass('completed');
		}
		...
		//SET ARROW AFTER DOWNLOADING CONTENT
	    var link = $(".nonVideoMaterial-" + result["IdContentType"]);
	    link.find(".additional-downloaded").addClass("show");
	    
		//CHECK IF USER CAN TAKE EXAM AND ENABLE EXAM BOX
	    for(var key in result["Exams"])
	    {
		   if (result["Exams"][key] != "")
		   {
			   var ahref = $(".selfAssessment-" + key)
			   ahref.attr("href",result["Exams"][key]);
			   ahref.find(".box").addClass("clickeable");
		   }
	    }
...
function RegisterSocialSharing(payload, idCourse, idModule) {
	if (lock(mutex)) {
		var params = {
			payload: payload,
			idCourse: idCourse,
		...
		CallAJAXPost("Handlers/WebMethodsHandler.aspx/RegisterSocialSharing", params, GenericSuccessHandler, GenericErrorHandler);
	}
}

//End region: Content download handlers
...
function showNextPanelEducationTypes_onSuccess(result) {
	...
	if(result["ModuleName"]) {
		trackNavigationClick('expander:' + result["ModuleName"], 'o');
	}
...
function SubscribeToET_onSuccess(result) {
	if (result)
	{
		...
		if (!isNaN(num))
		{
			var trackName = location.pathname.match(/\/training-courses\/(.*)/)[1];			
			var value = "m=" + result + "&e=" + ((examClick) ? examClick : "0") + "&ct=" + ((ctypeClick) ? ctypeClick : "0");
			
			saveCookie("track-" + trackName, value);
...
function RefreshTrackRatings() {
	//variable lnkTrackRatings is injected in EducationDetails page
	__doPostBack(lnkTrackRatings,'');
}
...
function nextQuestion_SuccessHandler(result) {
    if (result["Status"] == "NextQuestion") {
        ...
        finishExam(result["Message"], result["Facebook"], result["Twitter"]);
		if (result["SurveyLink"])
		{
			if (result["SurveyLink"] != "")
			{
				$("#linkSurveyLink").parent().show();
				$("#linkSurveyLink").attr("href",result["SurveyLink"]);
			}
		}
        ...
        if (result["Status"] == "Approved") {
			...
			if(result["ModuleName"]) {
				trackAssessmentCompleted(result["ModuleName"]);
			}
        }
...
function finishExam(message, facebook, twitter) {
    finishExamTimer();
...
/* Get value from a set of key-values */
function getParamValue(key, body, default_) {
	var keyIndex = body.indexOf(key + '=') + key.length + 1;
	if(keyIndex > key.length) {
		var endIndex = body.indexOf('&', keyIndex);
		if(endIndex > keyIndex) {
			return body.substr(keyIndex, endIndex - keyIndex);
		}
		else {
			return body.substr(keyIndex);
		}
	}
	else {
		return default_;
...
/* Get cookie by name */
function getCookieValue(name, default_){
	if(default_ == null){
		default_ = "";
	}
	var re = new RegExp(name + "=([^;]+)");
	var value = re.exec(document.cookie);
	return (value != null) ? unescape(value[1]) : default_;
}
...
window.open(url,target,'left=0,top=0' + ',fullscreen=1,toolbar=1,scrollbars=1,location=1,statusbar=1,menubar=1,resizable=1,status=1,titlebar=1');
}

function ShowActivity(scope) {
	if(scope == 'local') {
		$("div[id$='totalactivityLocal']").removeClass("hide");
		$("div[id$='totalactivityGlobal']").addClass("hide");
		$("#sb_communityinfo .global").removeClass("active");
		$("#sb_communityinfo .local").addClass("active");
	}
	if(scope == 'global') {
		$("div[id$='totalactivityGlobal']").removeClass("hide");
		$("div[id$='totalactivityLocal']").addClass("hide");
		$("#sb_communityinfo .local").removeClass("active");
		$("#sb_communityinfo .global").addClass("active");
	}
}

function TogglePanelScope(idParent, scope) {	
	if(scope == 'local') {
		$("div[id$='localData']").removeClass("hide");
		$("div[id$='globalData']").addClass("hide");
		$(idParent + " .tab .global").removeClass("active");
		$(idParent + " .tab .local").addClass("active");
	}
	if(scope == 'global') {
		$("div[id$='globalData']").removeClass("hide");
		$("div[id$='localData']").addClass("hide");
		$(idParent + " .tab .global").addClass("active");
		$(idParent + " .tab .local").removeClass("active");
	}
}

//Function to redirect to a topic page from the homepage
function goToTopicPage(topicPage)
{
	window.location = topicPage;
}
function goToTopicPageTouch(topicPage, element)
{
    $('.topic-callout').hide();
	window.location = topicPage;
	return false;
...
*/
function initializeTextPanel(controlID, lines) {
	var contentHeight = $("#" + controlID).height();
	var lineHeight = $("#" + controlID).css("line-height").replace('px', '');
...
function collapseTextPanel(controlID, lines) {
...
function expandTextPanel(controlID) {
	var contentHeight = $("#" + controlID + "-inner").height();
...
/*
	VIEW MORE COURSES
*/
function viewMoreCourses(controlID, height) {
	$('#' + controlID + ' .container-inner').css('height', height);
	$('.' + controlID + '-view-more-courses').css('display', 'none');
	$('.' + controlID + '-view-less-courses').css('display', 'inline-block');
}

function viewLessCourses(controlID, height) {
	$('#' + controlID + ' .container-inner').css('height', height);
	$('.' + controlID + '-view-less-courses').css('display', 'none');
	$('.' + controlID + '-view-more-courses').css('display', 'inline-block');
}

/*
    TRACKING
*/
function trackHomeBannerClick(bannerName, linkType, position) {
	s.linkTrackVars = "eVar36,eVar38,eVar66,events";
	s.linkTrackEvents = "event36,event38";
	s.eVar36 = "top banner:" + bannerName;
	s.eVar38 = bannerName;
	s.eVar66 = position;
	s.events = "event36,event38";
	s.tl(this, linkType, 'top banner:' + bannerName);
	return true;
}
function trackLiveEventBannerClick(liveEventName, linkType, position) {
	s.linkTrackVars = "eVar36,eVar38,eVar66,events";
	s.linkTrackEvents = "event36,event38";
	s.eVar36 = "right rail: event countdown:" + liveEventName;
	s.eVar38 = "right rail: event countdown:" + liveEventName;
	s.eVar66 = position;
	s.events = "event36,event38";
	s.tl(this, linkType, 'right rail: event countdown:' + liveEventName);
	return true;
}
function trackNavigationClick(navigationElement, linkType) {
	s.linkTrackVars = "eVar35,events";
	s.linkTrackEvents = "event35";
	s.eVar35 = navigationElement;
	s.events = "event35";
	s.tl(this, linkType, navigationElement);
	return true;
}
function trackFileDownloadClick(downloadTitle, downloadType, elementName) {
	s.linkTrackVars = "eVar50,eVar51,events";
	s.linkTrackEvents = "event50";
	s.eVar50 = downloadTitle;
	s.eVar51 = downloadType;
	s.events = "event50";
	s.tl(true, 'o', elementName);
	return true;
}
function trackContentLinks(contentElement, linkType) {
	s.linkTrackVars = "eVar36,events";
	s.linkTrackEvents = "event36";
	s.eVar36 = contentElement;
	s.events = "event36";
	s.tl(this, linkType, contentElement);
	return true;
}
function trackRecommendedTrackClick(trackName) {
	s.linkTrackVars = "eVar63,events";
...
function trackOpenContentClick(contentName) {
	s.linkTrackVars = "eVar36,events";
	s.linkTrackEvents = "event36";
	s.eVar36 = contentName;
	s.events = "event36";
	s.tl(this, 'd', contentName);
	return true;
}
function trackPartialEngagement(moduleName, meteorCampaignUUID) {
	s.linkTrackVars = "eVar44,events";
	s.linkTrackEvents = "event57";
	s.eVar44 = moduleName;
	s.events = "event57";
	s.tl(this, 'o', moduleName);
		
	//Meteor tagging
    meteor.tracking.track_conversion(meteorCampaignUUID,{'name' : 'partial engagement:' + moduleName});
	
	return true;
}
function trackFullEngagement(moduleName) {
	s.linkTrackVars = "eVar44,events";
...
function trackSearchClick(searchTerm) {
	...
	s.eVar40 = searchTerm;
	s.events = "event40";
	s.tl(this, 'o', 'search:' + searchTerm);
	return true;
}
function trackCertificateDownloadClick(trackName) {
	s.linkTrackVars = "eVar41,events";
	s.linkTrackEvents = "event62";
	s.eVar41 = trackName;
	s.events = "event62";
	s.tl(true, 'd', trackName);
	return true;
}
function trackTranscriptDownloadClick(trackName) {
	return true;
}
function trackAssessmentStarted(moduleName) {
	s.linkTrackVars = "eVar44,events";
	s.linkTrackEvents = "event59";
	s.eVar44 = moduleName;
	s.events = "event59";
	s.tl(true, 'o', moduleName);
	return true;
}
function trackAssessmentCompleted(moduleName) {
	s.linkTrackVars = "eVar44,events";
...
function trackFeedClick(feedType, feedName) {
	s.linkTrackVars = "eVar48,events";
	s.linkTrackEvents = "event48";
	s.events = "event48";
	s.eVar48 = feedType;
	s.tl(this, 'o', feedName);
	return true;
}
function trackRedeemVoucher(voucherCode) {
	s.linkTrackVars = "eVar70,events";
	s.linkTrackEvents = "event70";
	s.events = "event70";
	s.eVar70 = voucherCode;
	s.tl(true, 'o', 'voucher redeem:voucher redeem');
	return true;
}
function trackEventRegistration(eventName, code, meteorCampaignUUID) {
	if(code == '') {
		s.linkTrackVars = "eVar65,eVar36,events";
	}
	else {
		s.linkTrackVars = "eVar65,eVar36,eVar68,events";
		s.eVar68 = code;
	}
	s.linkTrackEvents = "event65,event36";
	s.events = "event65,event36";
	s.eVar65 = eventName;
	s.eVar36 = "registration button:" + eventName;

	s.tl(true, 'e', 'live events:event registration');
	
	//Meteor tagging
    meteor.tracking.track_conversion(meteorCampaignUUID,{'name' : 'live events:event registration:' + eventName});
	
	return true;
}
function trackLiveEventRegistration(eventName, code) {
	if(code == '') {
		s.linkTrackVars = "eVar36,events";
	}
	else {
		s.linkTrackVars = "eVar36,eVar68,events";
		s.eVar68 = code;
	}
	s.linkTrackEvents = "event36";
	s.events = "event36";
...
function trackLiveEventFeedback(eventName) {
	s.linkTrackVars = "eVar66,events";
	s.linkTrackEvents = "event66";
	s.events = "event66";
	s.eVar66 = eventName;
	s.tl(this, 'o', 'live events:event feedback');
	return true;
}
function trackSocialEvents(eventName) {
	s.linkTrackVars = "eVar56,events";
	s.linkTrackEvents = "event56";
	s.events = "event56";
	s.eVar56 = eventName;
	s.tl(this, 'e', 'social event:' + eventName);
	return true;
}
function trackCourseCompletion(courseName) {
	s.linkTrackVars = "eVar41,events";
	s.linkTrackEvents = "event61";
	s.events = "event61";
	s.eVar41 = courseName;
	s.tl(true, 'o', 'course completion:' + courseName);
	return true;
}
function trackCourseRating(courseName, value) {
	s.linkTrackVars = "eVar41,eVar64,events";
	s.linkTrackEvents = "event64";
	s.events = "event64";
	s.eVar41 = courseName;
	s.eVar64 = value + " star";
	s.tl(true, 'o', 'course rating:' + courseName);
	return true;
}
function trackDashboardAlertClosing(alertName) {
	s.linkTrackVars = "eVar74,events";
	s.linkTrackEvents = "event75";
	s.events = "event75";
	s.eVar74 = alertName;
	s.tl(true, 'o', 'close dashboard alert:' + alertName);
	return true;
}
function triggerDashboardAlertTracking(name, linkType) {
	s.linkTrackVars = "eVar72,events";
...
function trackVideoClick(videoName, courseName, meteorCampaignUUID) {
	s.linkTrackVars = "eVar52,eVar53,events";
	s.linkTrackEvents = "event49";
	s.eVar52 = videoName;
	s.eVar53 = videoName + ':' + courseName;
	s.events = "event49";
	s.tl(true, 'd', videoName);
	
	//Meteor tagging
    meteor.tracking.track_conversion(meteorCampaignUUID,{'name' : 'video click:' + videoName});
	
	return true;
}
function trackVideoChapterClick(videoName, courseName, chapterName, meteorCampaignUUID) {
	s.linkTrackVars = "eVar52,eVar53,eVar54,events";
	s.linkTrackEvents = "event49,event51";
	s.eVar52 = videoName;
	s.eVar53 = videoName + ':' + courseName;
	s.eVar54 = chapterName + ':' + videoName;
	s.events = "event49,event51";
	s.tl(true, 'd', videoName);
	
	//Meteor tagging
    meteor.tracking.track_conversion(meteorCampaignUUID,{'name' : 'video click:' + videoName + ':' + chapterName});
	
	return true;
}
function trackHomepageButtonLinks(contentElement, linkType) {
	s.linkTrackVars = "eVar36,events";
	s.linkTrackEvents = "event36,event68";
	s.eVar36 = contentElement;
	s.events = "event36,event68";
	s.tl(this, linkType, contentElement);
	return true;
}
function trackRelatedCourseLinks(courseTitle, linkType) {
	s.linkTrackVars = "eVar36,eVar34,events";
	s.linkTrackEvents = "event36,event34";
	s.eVar36 = "related courses:" + courseTitle;
	s.eVar34 = courseTitle;
	s.events = "event36,event34";
	s.tl(this, linkType, courseTitle);
	return true;
}
function trackOpenFreeActionsPopUp(popUpName, linkType) {
	s.linkTrackVars = "eVar71,events";
	s.linkTrackEvents = "event79";
	s.events = "event79";
	s.eVar71 = popUpName;
	s.tl(this, linkType, popUpName);
	return true;
}
function trackFreeActionPopUpClick(actionName, linkType) {
	s.linkTrackVars = "eVar36,events";
	s.linkTrackEvents = "event36";
	s.events = "event36";
	s.eVar36 = actionName;
	s.tl(this, linkType, actionName);
	return true;
}
function trackLearningPlanWidgetClick(navigationElement, linkType) {
	s.linkTrackVars = "eVar35,events";
	s.linkTrackEvents = "event35,event83";
	s.events = "event35,event83";
	s.eVar35 = navigationElement;
	s.tl(true, linkType, navigationElement);
	return true;
}
function trackLearningPlanClick(navigationElement, courseName, learningPlanName, learningPlanSource, linkType) {
	s.linkTrackVars = "eVar35,eVar4,eVar6,eVar7,eVar41,events";
	s.linkTrackEvents = "event35,event88";
	s.events = "event35,event88";
	s.eVar35 = navigationElement;
	s.eVar4 = courseName;
	s.eVar6 = learningPlanName;
	s.eVar7 = learningPlanSource;
	s.eVar41 = courseName;
	s.tl(true, linkType, navigationElement);
	return true;
}
function trackDashboardLearningPlanClick(navigationElement, courseName, learningPlanName, learningPlanSource, linkType) {
	s.linkTrackVars = "eVar35,eVar4,eVar6,eVar7,eVar41,events";
	s.linkTrackEvents = "event35,event88";
	s.events = "event35,event88";
	s.eVar35 = navigationElement;
...
function trackSignInNavigationClick(navigationElement, linkType) {
	s.linkTrackVars = "eVar35,events";
	s.linkTrackEvents = "event35";
	s.eVar35 = navigationElement;
	s.events = "event35";
...
function trackCourseAddedToLearningPlan(navigationElement, linkType) {
	s.linkTrackVars = "eVar4,events";
	s.linkTrackEvents = "event84";
	s.eVar4 = navigationElement;
	s.events = "event84";
	s.tl(true, linkType, navigationElement);
	return true;
}
function trackAddedLearningPlan(learningPlanName, learningPlanType, linkType) {
	s.linkTrackVars = "eVar6,eVar7,events";
	s.linkTrackEvents = "event86";
	s.eVar6 = learningPlanName;
	s.eVar7 = learningPlanType;
	s.events = "event86";
	s.tl(true, linkType, learningPlanName);
	return true;
}
function trackLearningPlanDeleted(learningPlanName, learningPlanType, linkType) {
	s.linkTrackVars = "eVar6,eVar7,events";
...
function trackSearchResultClick(contentName, linkType) {
	s.linkTrackVars = "eVar36,events";
	s.linkTrackEvents = "event81";
	s.eVar36 = contentName;
	s.events = "event81";
	s.tl(true, linkType, contentName);
	return true;
}

function trackOfferRegistrationClick(offerName){
	s.linkTrackVars = "eVar36,eVar9,events";
	s.linkTrackEvents = "event36,event90";
	s.eVar9 = offerName;
	s.eVar36 = "offers:"+offerName;
	s.events = "event36,event90";
	s.tl(true,'o',offerName);
	return true;
}

function trackUploadToDegreedClick(){
...
/*
	METEOR TAGGING
*/
function trackMeteorSocialShareClick(socialNetwork, message, campaginUUID, conversionName, title) {
	s.linkTrackVars="eVar56,events";
	s.linkTrackEvents="event56";
	s.eVar56="meteor " + socialNetwork + " share";
	s.events="event56";
	s.tl(true,'o','meteor ' + socialNetwork + ' share');

    if(title) 
		meteor.sharing.superbar.trigger(socialNetwork, {'title': title, 'message': message}); 
	else 
		meteor.sharing.superbar.trigger(socialNetwork, {'message': message}); 

		meteor.tracking.track_conversion(campaginUUID,{'name' : conversionName});
}
...
function initializeCarousel(numberOfElements, interval, speed, baseName, showAction, automaticAnimation) {
	//Only animate when there is more than one element.
	if (numberOfElements <= 1) return;
	
	//Load array
	referenceArray[baseName] = baseName;
	
	//Forces the first load
	$(baseName + ' .carousel-element').hide();
	$(baseName + ' .carousel-element').first().show();
	
	//Execute only if action execution is desired
	if(showAction) {	
		//Appends the control buttons
		html = "";
		for (var i = 0; i < numberOfElements; i++) {
			html += "<div carousel_index='" + i + "' class='carousel_action_button button_" + (i) + " '></div>";
		}
		$(baseName + ' .carousel_action').append(html);

		//Sets the action for each button
		$(baseName + ' .carousel_action').find(".carousel_action_button").click(function () {
			index = $(this).attr("carousel_index");
			
			//Avoid re-charging - clear everything
			clearTimeout(referenceArray[baseName]);
			
			//Re-initiate a cycle starting from the index clicked
			$(baseName + ' .carousel-element:visible').fadeOut(speed); 
			$(baseName + ' .carousel-element.item-' + index).fadeIn(speed); 
			//Resets all other buttons
			$(baseName + ' .carousel_action_button').removeClass('active');
			//Sets the CLICKED button as active
			$(baseName + ' .carousel_action_button.button_' + index).addClass('active');
		});
	}

	//Resets all other buttons (just in case)
	$(baseName + ' .carousel_action_button').removeClass('active');
	//Sets the FIRST button as active
	$(baseName + ' .carousel_action_button.button_0').addClass('active');

	if(automaticAnimation) {
		//currentElement: 0
		referenceArray[baseName] = setTimeout(function () { animateCarousel(0, numberOfElements, interval, speed, baseName); }, interval);
	}
}

function animateCarousel(currentElement, numberOfElements, interval, speed, baseName) {
	$(baseName + ' .carousel-element.item-' + currentElement).fadeOut(speed);
	...
	$(baseName + ' .carousel-element.item-' + currentElement).fadeIn(speed);
	
	//Resets all other buttons
	$(baseName + ' .carousel_action_button').removeClass('active');
	//Sets the CURRENT button as active
	$(baseName + ' .carousel_action_button.button_' + currentElement).addClass('active');

	//Sets next iteration
	referenceArray[baseName] = setTimeout(function () { animateCarousel(currentElement, numberOfElements, interval, speed, baseName); }, interval);
}

/* COURSES HOVER MENU */
var mergedCoursesHoverMenu;

function bindTopNavCoursesMenu()
{
	var menuItem = $('a[name="header:courses"]'),
		hoverMenu = $("[id$='topNavCoursesHoverMenu']");
	
	menuItem.removeAttr('onclick');
	
	menuItem.bind('click', function () {
		return showTopNavCoursesMenu();
	});
	
	mergedCoursesHoverMenu = $.merge(menuItem, hoverMenu);
	
	mergedCoursesHoverMenu.bind('mouseenter', function () {
		return showTopNavCoursesMenu();
	});
	
	mergedCoursesHoverMenu.bind('mouseleave', function () {
		return hideTopNavCoursesMenu();
	});
	
	return false;
}
function showTopNavCoursesMenu()
{
	$("[id$='topNavCoursesHoverMenu']").css('display', 'block');
	
	return false;
}
function hideTopNavCoursesMenu()
{
	$("[id$='topNavCoursesHoverMenu']").css('display', 'none');
	
	return false;
}

/* HELP HOVER MENU */
var mergedHelpHoverMenu;

function bindTopNavHelpMenu()
{
	var menuItem = $('a[name="header:help"]'),
		hoverMenu = $("[id$='topNavHelpHoverMenu']");
	
	menuItem.removeAttr('onclick');
	
	menuItem.bind('click', function () {
		return showTopNavHelpMenu();
	});
	
	mergedHelpHoverMenu = $.merge(menuItem, hoverMenu);
	
	mergedHelpHoverMenu.bind('mouseenter', function () {
		return showTopNavHelpMenu();
	});
	
	mergedHelpHoverMenu.bind('mouseleave', function () {
		return hideTopNavHelpMenu();
	});
	
	return false;
}
function showTopNavHelpMenu()
{
	$("[id$='topNavHelpHoverMenu']").css('display', 'block');
	
	return false;
}
function hideTopNavHelpMenu()
{
	$("[id$='topNavHelpHoverMenu']").css('display', 'none');
...
/******* MY LEARNING PLAN *******/
function initWidget() {
	expandedPlans = $('#hfCollapsedPlans').val();

	//Reset all
	$('#LoggedIn .learning-plan .learning-plan-icon').addClass('closed');
	$('#LoggedIn .learning-plan .learning-plan-icon').removeClass('opened');
	$('#LoggedIn .learning-plan .learning-plan-description').hide();

	if (expandedPlans == '||') {
		$('#LoggedIn .learning-plan').first().find('.learning-plan-icon').removeClass('closed');
		$('#LoggedIn .learning-plan').first().find('.learning-plan-icon').addClass('opened');
		$('#LoggedIn .learning-plan').first().find('.learning-plan-description').show();
	}
	else {
		$('.learning-plan-description').each(function () {
			thisId = $(this).attr('id').substr(14);
			allIds = $('#hfCollapsedPlans').val();

			if (allIds.indexOf('|' + thisId + '|') != -1) {
				//Learning plan expanded
				$('#LoggedIn .learning-plan #learning-plan-icon' + thisId).removeClass('closed');
				$('#LoggedIn .learning-plan #learning-plan-icon' + thisId).addClass('opened');
				$(this).show();
			}
		});
	}

	$('#MyLearningPlanWidget .truncable').dotdotdot();
}

function addExpandedLearningPlan(newLearningPlanIds) {
	allIds = $('#hfCollapsedPlans').val();

	$('#hfCollapsedPlans').val('||');
	
	if (newLearningPlanIds.length > 0) {
		newLearningPlanIds.forEach(function (elem) {
			allIds += elem;
			allIds += '|';
		});

		$('#hfCollapsedPlans').val(allIds);
	}
}

/*
    JQUERY - DEFAULT VALUE
    Implement a default value for text boxes that dissapear on focus and blur.
*/
...
function getIDsArray(arrayName, itemPrefix) {
